...
fn test_vehicle_example_all_references_have_hover() {
    ...
    println!("\n[DEBUG] Looking for ignitionCmd type_refs...");
    {
        ...
        for r in &all_refs {
            if r.source_symbol
                .as_deref()
                .map(|s| s.contains("ignitionCmd"))
                .unwrap_or(false)
            {
                println!(
                    "[DEBUG] ignitionCmd ref: target='{}' line={} col {}-{}",
    ...
    let mut all_refs: Vec<(String, String, u32, u32, u32)> = refs
        ...
        .map(|r| {
            (
                r.target,
                r.source_symbol.unwrap_or_default(),
                r.start_line,
                r.start_col,
                r.end_col,
            )
    ...
    for (target, source, line, col_start, col_end) in &all_refs {
        ...
        if line_text.contains("message") && line_text.contains("from") {
            ...
            println!(
                "[DEBUG]   hover_result={:?}",
                hover_result.as_ref().map(|h| format!("{:?}", h.contents))
            );
        }
    ...
    if !failing_by_pattern.is_empty() {
        ...
        for (pattern, refs) in patterns {
            ...
            for (i, r) in refs.iter().take(5).enumerate() {
                ...
                println!(
                    "     Target: '{}' at col {}-{}",
                    r.target, r.col_start, r.col_end
                );
...
fn test_vehicle_example_semantic_tokens() {
    ...
    let namespace_tokens: Vec<_> = decoded.iter().filter(|(_, _, _, tt)| *tt == 0).collect();
    ...
    let line0_tokens: Vec<_> = decoded.iter().filter(|(l, _, _, _)| *l == 0).collect();

    assert!(
        !line0_tokens.is_empty(),
        "Should have at least one token on line 0"
    );
    ...
    assert_eq!(
        *col, 8,
        "SimpleVehicleModel should start at col 8, got {}",
        col
    );
    assert_eq!(*len, 18, "SimpleVehicleModel has 18 chars, got {}", len);

    // Helper to find token covering a specific position
    let _find_token_at = |target_line: u32, target_col: u32| -> Option<(u32, u32, u32, u32)> {
        decoded
            .iter()
    ...
    let assert_token_at = |line_num: u32, expected_col: u32, expected_text: &str, desc: &str| {
        ...
        let tokens_on_line: Vec<_> = decoded
            .iter()
            .filter(|(l, _, _, _)| *l == line_num)
            .collect();

        // Find token at expected column
        let token = tokens_on_line
            .iter()
            .find(|(_, col, _, _)| *col == expected_col);

        match token {
            Some((_, col, len, _)) => {
                let start = *col as usize;
                let end = (start + *len as usize).min(source_line.len());
                let actual_text = if start < source_line.len() {
                    &source_line[start..end]
                } else {
                    ""
                };
            ...
            None => {
                let available: Vec<String> = tokens_on_line
                    .iter()
                    .map(|(_, col, len, _)| {
                        let start = *col as usize;
                        let end = (start + *len as usize).min(source_line.len());
                        let text = if start < source_line.len() {
                            &source_line[start..end]
                        } else {
                            "<oor>"
                        };
