...
pub trait LspServerTestExt {
    ...
    fn has_file_path(&self, path: &PathBuf) -> bool;
    ...
    fn reference_at_position(&mut self, file_path: &str, line: u32, col: u32) -> Option<TypeRefSnapshot>;
...
impl LspServerTestExt for LspServer {
    ...
    fn has_symbol(&mut self, name: &str) -> bool {
        let analysis = self.analysis_host.analysis();
        analysis.symbol_index().all_symbols().any(|s| s.name.as_ref() == name)
    }
    
    fn has_qualified_symbol(&mut self, qname: &str) -> bool {
        let analysis = self.analysis_host.analysis();
        analysis.symbol_index().all_symbols().any(|s| s.qualified_name.as_ref() == qname)
    }
    
    fn symbol_names(&mut self) -> Vec<String> {
        let analysis = self.analysis_host.analysis();
        analysis.symbol_index().all_symbols().map(|s| s.name.to_string()).collect()
    }
    
    fn qualified_symbol_names(&mut self) -> Vec<String> {
        let analysis = self.analysis_host.analysis();
        analysis.symbol_index().all_symbols().map(|s| s.qualified_name.to_string()).collect()
    }
    ...
    {
        let analysis = self.analysis_host.analysis();
        analysis.symbol_index().all_symbols()
    ...
    fn reference_at_position(&mut self, file_path: &str, line: u32, col: u32) -> Option<TypeRefSnapshot> {
        self.references_in_file(file_path)
            .into_iter()
            .find(|r| {
    ...
    fn all_reference_targets(&mut self) -> Vec<String> {
        let analysis = self.analysis_host.analysis();
        let mut targets: Vec<_> = analysis.symbol_index().all_symbols()
            .flat_map(|s| s.type_refs.iter())
            .flat_map(|trk| trk.as_refs())
