    ...
    pub fn find_symbol_at_position(
        ...
        for sym in analysis.symbol_index().symbols_in_file(file_id) {
            for type_ref_kind in &sym.type_refs {
                ...
                if let Some((_, type_ref)) = type_ref_kind.part_at(position.line, position.character) {
                    let range = Range {
                        start: Position {
        ...
        for sym in analysis.symbol_index().symbols_in_file(file_id) {
            if contains_position(sym.start_line, sym.start_col, sym.end_line, sym.end_col, position.line, position.character) {
                let range = Range {
                    start: Position {
                        line: sym.start_line,
...
fn contains_position(start_line: u32, start_col: u32, end_line: u32, end_col: u32, line: u32, col: u32) -> bool {
