...
impl LspServer {
    ...
    pub fn get_hover(&mut self, uri: &Url, position: Position) -> Option<Hover> {
        ...
        if let Some(qualified_name) = result.qualified_name.as_ref() {
            contents = Self::add_references_section_from_analysis(&analysis, &contents, qualified_name);
        }
    ...
    fn add_references_section_from_analysis(analysis: &syster::ide::Analysis<'_>, content: &str, qualified_name: &str) -> String {
        ...
        let mut references: Vec<_> = analysis.symbol_index().all_symbols()
            .filter(|sym| {
                sym.type_refs.iter()
                    .flat_map(|trk| trk.as_refs())
                    .any(|tr| tr.target.as_ref() == qualified_name || tr.target.as_ref() == simple_name)
            })
            .flat_map(|sym| {
                sym.type_refs.iter()
                    .flat_map(|trk| trk.as_refs())
                    .filter(|tr| tr.target.as_ref() == qualified_name || tr.target.as_ref() == simple_name)
